    ...
    <Paper variant="outlined" sx={{ p: compact ? 2 : 2.5, height: "100%", overflow: "auto" }}>
      <Stack direction={compact ? "column" : { xs: "column", md: "row" }} spacing={compact ? 6 : 7} sx={{ alignItems: compact ? "stretch" : { md: "center" } }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: `repeat(${compact ? 3 : 4}, minmax(${compact ? 86 : 130}px, 1fr))`, gridTemplateRows: `repeat(${compact ? 7 : 3}, auto)`, gap: 1.25, flex: 1, maxWidth: 800 }}>
          ...
          {compact ? <Box sx={{ gridColumn: "1 / -1", gridRow: 4, display: "flex", alignItems: "center", gap: 1, color: "text.secondary" }}><Box sx={{ flex: 1, borderTop: "1px solid", borderColor: "divider" }} /><Typography variant="caption" sx={{ fontWeight: 750, letterSpacing: "0.08em" }}>Z-ACHSE</Typography><Box sx={{ flex: 1, my: 8, borderTop: "1px solid", borderColor: "divider" }} /></Box> : null}
